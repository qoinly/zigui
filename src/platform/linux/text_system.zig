...
pub const LinuxTextSystem = struct {
    ...
    fn layout_line_impl(
        ...
        const shaped = hb.shape(e.hb_font, self.hb_buffer.?, text) orelse
            return whitespace_layout(e, font_size);
        const n: u32 = @min(shaped.len, MAX_GLYPHS);
        ...
        while (i < n) : (i += 1) {
            ...
            self.glyph_buffer.append(self.allocator, .{
                ...
                .position = .{
                    .x = (pen_units + @as(f32, @floatFromInt(pos.x_offset))) * scale,
                    .y = 0,
                },
                .index = info.cluster,
                .is_emoji = false,
            }) catch break;
            pen_units += @floatFromInt(pos.x_advance);
            var extents: hb.GlyphExtents = undefined;
            if (hb.glyph_extents(e.hb_font, info.codepoint, &extents)) {
                // hb extents grow downward: height is negative, bearing is the top.
                const bottom = -(extents.y_bearing + extents.height);
                ink_top = @max(ink_top, @as(f32, @floatFromInt(extents.y_bearing)));
                ink_bottom = @max(ink_bottom, @as(f32, @floatFromInt(bottom)));
            }
        }
    ...
    fn sized_face(self: *Self, params: ts.RenderGlyphParams) ?*ft.Face {
        ...
        if (e.size_px != px) {
            if (!ft.set_pixel_size(e.ft_face, px)) return null;
            e.size_px = px;
        }
        return e.ft_face;
    }
